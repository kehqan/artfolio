...
export default async function PublicProfilePage({
  ...
  const [
    ...
    supabase
      .from("artworks")
      .select("id, title, status, images, price, medium, year")
    ...
    <div style={{ minHeight: "100vh", background: "#FFFBEA" }}>
      ...
      <nav style={{
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <Link href="/login" style={{ textDecoration: "none" }}>
            <button style={{ padding: "7px 14px", border: "2px solid #444", borderRadius: 6, background: "transparent", color: "#aaa", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
              Sign in
            </button>
          </Link>
          <Link href="/register" style={{ textDecoration: "none" }}>
            <button style={{ padding: "7px 14px", border: "2px solid #FFD400", borderRadius: 6, background: "#FFD400", color: "#111110", fontWeight: 800, fontSize: 12, cursor: "pointer" }}>
              Join free
            </button>
          </Link>
        </div>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 24px 64px" }}>
        ...
        <div style={{
          ...
          borderBottom: "2px solid #111110", marginBottom: 40,
          flexWrap: "wrap",
        }}>
          {/* Avatar */}
          <div style={{ flexShrink: 0 }}>
            {profileAvatar ? (
              ...
              <img
                src={profileAvatar}
                alt={profileName}
                style={{ width: 110, height: 110, borderRadius: "50%", objectFit: "cover", border: "3px solid #111110", boxShadow: "4px 4px 0 #111110" }}
              />
            ) : (
          ...
          <div style={{ flex: 1, minWidth: 260 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8, flexWrap: "wrap" }}>
              <h1 style={{ fontSize: 28, fontWeight: 900, color: "#111110", letterSpacing: "-0.5px", margin: 0 }}>
                {profileName}
              </h1>
              {profileRole && (
                <span style={{ padding: "3px 12px", border: "2px solid #111110", borderRadius: 20, fontSize: 11, fontWeight: 800, background: "#FFD400", color: "#111110", textTransform: "capitalize" }}>
                  {profileRole}
                </span>
              )}
            </div>
            ...
            {profileBio && (
              <p style={{ fontSize: 14, color: "#5C5346", lineHeight: 1.7, maxWidth: 540, marginBottom: 20 }}>
                {profileBio}
              </p>
            )}
        ...
        {artworks.length > 0 && (
          <section style={{ marginBottom: 52 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14 }}>
              {artworks.map((a) => {
                const cfg = STATUS_CFG[a.status as string ?? ""] ?? { bg: "#F5F0E8", color: "#5C5346" };
                const imgs = Array.isArray(a.images) ? a.images as string[] : [];
                const img  = imgs[0] ?? null;
                return (
                  <div key={a.id} style={{ background: "#fff", border: "2px solid #111110", borderRadius: 8, overflow: "hidden", boxShadow: "3px 3px 0 #111110" }}>
                    ...
                    <div style={{ padding: "10px 12px" }}>
                      <div style={{ fontSize: 13, fontWeight: 700, color: "#111110", marginBottom: 6, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                        {a.title as string}
                      </div>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        ...
                        {typeof a.price === "number" && String(a.status).toLowerCase() === "available" && (
                  ...
                  <div style={{ padding: "12px 14px" }}>
                    ...
                    {(ex.start_date || ex.end_date) && (
                      <div style={{ fontSize: 11, color: "#9B8F7A", marginTop: 2 }}>
                        {fmtDate(ex.start_date as string | null)}
                        {ex.end_date ? ` — ${fmtDate(ex.end_date as string | null)}` : ""}
                      </div>
                    )}
              ...
              {collabs.map((c) => (
                <div key={c.id} style={{ background: "#fff", border: "2px solid #111110", borderRadius: 8, padding: "14px 16px", boxShadow: "3px 3px 0 #111110" }}>
                  <div style={{ display: "flex", alignItems: "flex-start", gap: 8, marginBottom: 8 }}>
                    ...
                    {c.type && (
                      <span style={{ fontSize: 9, fontWeight: 800, padding: "2px 7px", border: "1.5px solid #111110", borderRadius: 3, background: "#F5F0E8", color: "#5C5346", textTransform: "uppercase", flexShrink: 0 }}>
                        {c.type as string}
                      </span>
                    )}
                  </div>
        ...
        {artworks.length === 0 && exhibitions.length === 0 && collabs.length === 0 && (
          <div style={{ padding: "64px 24px", textAlign: "center" }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 800, color: "#111110", marginBottom: 6 }}>
              {profileName}&apos;s portfolio is taking shape
            </div>
