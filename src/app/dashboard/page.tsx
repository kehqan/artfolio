...
// artworks.images is a string[] in your schema — NOT image_url
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      // Fixed column names to match your actual Supabase schema:
      //   artworks    → user_id  (not artist_id)
      //   artworks    → images[] (not image_url)
      //   collections → user_id
      //   exhibitions → user_id
      //   follows     (table name, not "followers") → following_id
      //   clients     → user_id
      const [profRes, artRes, colRes, exhRes, folRes, cliRes] = await Promise.all([
        ...
        supabase.from('artworks')
          .select('id, title, medium, price, status, images, created_at')
          .eq('user_id', user.id)
      ...
      setStats({
        artworks:    artRes.count ?? rawArtworks.length,
    ...
    <>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <div style={{
          ...
          padding: '14px 28px', borderBottom: '2px solid #111110',
          background: '#FFFBEA', position: 'sticky', top: 0, zIndex: 10,
        }}>
          <div>
            <div style={{ fontSize: 22, fontWeight: 900, letterSpacing: '-0.5px', color: '#111110' }}>Dashboard</div>
            <div style={{ fontSize: 13, color: '#9B8F7A', marginTop: 2 }}>
              Welcome back{profile ? `, ${displayName}` : ''} — here&apos;s what&apos;s happening.
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <button className="db-topbar-btn" title="Search" style={{
            ...
            <button className="db-topbar-btn" title="Notifications" style={{
              width: 36, height: 36, border: '2px solid #111110', background: '#fff',
              boxShadow: '3px 3px 0 #111110', borderRadius: 6,
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: 15, cursor: 'pointer', transition: 'transform 0.1s, box-shadow 0.1s',
            ...
            <Link href="/dashboard/artworks/new">
              <button className="db-topbar-btn" style={{
            ...
            {statCards.map((s, i) => (
              <div key={s.label} className="db-stat-card" style={{
                ...
                <div>
                  ...
                  <div style={{ fontSize: 11, fontWeight: 600, marginTop: 4, color: s.positive ? '#00C853' : '#9B8F7A' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 18 }}>
            ...
            <div>
              <div style={{
                ...
                <div style={{
                  ...
                  Recent Artworks
                  <Link href="/dashboard/artworks" className="db-card-link" style={{
                ...
                {loading ? (
                  ...
                  artworks.map((art, idx) => {
                    const imageUrl = art.images?.[0] ?? null
                    return (
                      <div key={art.id} style={{
                        ...
                        {imageUrl ? (
                          <div style={{ marginBottom: 12 }}>
                            ...
                            <img src={imageUrl} alt={art.title} style={{
                        ...
                        <span style={{
                          ...
                          background: art.status?.toLowerCase() === 'sold' ? '#111110' : '#FFD400',
                          color: art.status?.toLowerCase() === 'sold' ? '#fff' : '#111110',
                          marginBottom: 10,
                        }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 16, fontSize: 12, fontWeight: 600, color: '#5C5346' }}>
                          <div className="db-artwork-action" style={{ display: 'flex', alignItems: 'center', gap: 5, cursor: 'pointer', transition: 'color 0.1s' }}>💬 <span>0</span></div>
                          <div className="db-artwork-action" style={{ display: 'flex', alignItems: 'center', gap: 5, cursor: 'pointer', transition: 'color 0.1s' }}>♡ <span>0</span></div>
                          <div className="db-artwork-action" style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 5, cursor: 'pointer', transition: 'color 0.1s' }}>↗ Share</div>
                        </div>
                      </div>
                    )
                  })
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{
                ...
                <div style={{ padding: '14px 18px', borderBottom: '2px solid #111110', fontSize: 14, fontWeight: 800, color: '#111110' }}>
                  Quick Actions
                </div>
                <div>
                  ...
                  {[
                    ...
                    <Link key={item.href} href={item.href} style={{ textDecoration: 'none' }}>
                      <div className="db-qaction" style={{
                        display: 'flex', alignItems: 'center', justifyContent: 'space-between',
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  Collectors & Clients
                  <Link href="/dashboard/clients" className="db-card-link" style={{
                    fontSize: 12, fontWeight: 700, color: '#111110', textDecoration: 'none',
                    display: 'flex', alignItems: 'center', gap: 4,
                    padding: '4px 10px', border: '2px solid #111110', borderRadius: 20,
                    background: '#FFD400', boxShadow: '2px 2px 0 #111110',
                ...
                <div>
                  {loading ? (
                    ...
                    <div style={{ padding: '24px 18px', textAlign: 'center', color: '#9B8F7A', fontSize: 13 }}>
                      No clients yet.{' '}
                      <Link href="/dashboard/clients" style={{ color: '#111110', fontWeight: 700 }}>Add one →</Link>
                    </div>
                  ) : (
                    clients.map((c, i) => (
                      <div key={c.id} style={{
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ fontSize: 11, color: '#9B8F7A', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{c.email ?? '—'}</div>
                        </div>
                        <span style={{
