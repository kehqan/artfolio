...
import {
  ...
  Handshake, ShoppingBag, User, Clock, Check, CheckCheck,
  Sparkles, ArrowLeft,
} from "lucide-react";

// ── Types ──────────────────────────────────────────────────────────
type Profile = { id: string; full_name: string; username?: string; avatar_url?: string; role?: string; };
...
const TEMPLATES = [
  { key: "interested",  label: "Interested!",        body: "Hi! I'm interested in collaborating. Can we set up a time to chat?" },
  { key: "love_work",   label: "Love your work",     body: "I came across your work and I absolutely love it. I'd love to connect!" },
  { key: "available",   label: "I'm available",      body: "Thanks for reaching out! I'm available and would love to discuss this further." },
  { key: "price",       label: "Price inquiry",      body: "Could you tell me more about the pricing for this piece?" },
  { key: "visit",       label: "Studio visit",       body: "I'd love to arrange a studio visit. What dates work best for you?" },
...
const CONTEXT_ICONS: Record<string, React.ReactNode> = {
  collab:           <Handshake size={11} />,
  artwork_inquiry:  <ShoppingBag size={11} />,
...
export default function MessagesPage() {
  ...
  useEffect(() => {
    ...
    sb.auth.getUser().then(({ data: { user } }) => {
      if (!user) return;
      setMyId(user.id);
      sb.from("profiles").select("id,full_name,username,avatar_url,role").eq("id", user.id).single()
  ...
  const loadMessages = useCallback(async (convoId: string) => {
    ...
    setMsgLoading(false);
    // Mark as read: refresh convo list
    loadConvos();
          ...
          <div className="msg-list">
            {loading ? (
              ...
              <div className="msg-empty">
                ...
                <div style={{ fontSize: 12, color: "#9B8F7A", fontWeight: 600 }}>
              ...
              const ctxType = c.context_type || "direct";
              return (
                <button
                  key={c.id}
                  className={`msg-convo-item ${isActive ? "active" : ""} ${c.unread_count > 0 ? "unread" : ""}`}
                  onClick={() => openConvo(c)}
                >
                  <Avatar profile={c.other_profile} guest={c.guest_name || undefined} size={40} />
                  <div className="msg-convo-info">
                    ...
                    {c.context_title && (
                      <div className="msg-convo-ctx">
                        {CONTEXT_ICONS[ctxType]}
                        {c.context_title}
                      </div>
                    )}
          ...
          {!active ? (
            ...
            <>
              ...
              <div className="msg-chat-hdr">
                ...
                <Avatar profile={active.other_profile} guest={active.guest_name || undefined} size={36} />
                <div className="msg-chat-hdr-info">
                  <div className="msg-chat-hdr-name">
                    {active.other_profile?.full_name || active.guest_name || "Guest"}
                  </div>
                  {active.context_title && (
                    <div className="msg-chat-hdr-ctx">
                      {CONTEXT_ICONS[active.context_type || "direct"]}
                      {active.context_title}
                    </div>
                  )}
                  {active.guest_email && (
                    <div className="msg-chat-hdr-ctx">
                      <User size={11} /> {active.guest_email}
                    </div>
                  )}
                </div>
                {active.other_profile?.username && (
                  <a
                    href={`/${active.other_profile.username}`}
                    target="_blank"
                  ...
                  <>
                    {messages.map((msg, i) => {
                      ...
                      const senderName = mine
                        ? "You"
                        : (active.other_profile?.full_name || msg.guest_name || "Guest");
                      ...
                      return (
                        <div key={msg.id}>
                          ...
                          <div className={`msg-bubble-row ${mine ? "mine" : "theirs"}`}>
                            {!mine && <Avatar profile={active.other_profile} guest={active.guest_name || undefined} size={28} />}
              ...
              <div className="msg-input-area">
                <button
                  className={`msg-template-toggle ${showTemplates ? "active" : ""}`}
...
const CSS = `
  ...
  .msg-search-input {
    ...
    font-family: inherit; font-size: 13px; font-weight: 600; color: #111110;
  }
  ...
  .msg-convo-item.active { background: #FFD400; box-shadow: 2px 2px 0 #111110; border: 2px solid #111110; }
  ...
  .msg-convo-top { display: flex; justify-content: space-between; align-items: baseline; gap: 4px; margin-bottom: 2px; }
  .msg-convo-name { font-size: 14px; font-weight: 800; color: #111110; truncate: ellipsis; white-space: nowrap; overflow: hidden; max-width: 140px; }
  .msg-convo-time { font-size: 10px; font-weight: 700; color: #9B8F7A; flex-shrink: 0; }
  .msg-convo-ctx {
    display: flex; align-items: center; gap: 3px;
    font-size: 10px; font-weight: 700; color: #9B8F7A;
  ...
  .msg-chat-hdr {
    ...
    padding: 14px 18px; border-bottom: 2px solid #E8E0D0;
  ...
  .msg-chat-hdr-name { font-size: 15px; font-weight: 900; color: #111110; }
  .msg-chat-hdr-ctx {
  ...
  .msg-profile-link {
    ...
    padding: 6px 12px; border: 2px solid #E8E0D0; border-radius: 9px;
    text-decoration: none; white-space: nowrap; transition: all .15s;
    background: #fff;
  }
  .msg-profile-link:hover { border-color: #111110; background: #FFFBEA; }
  ...
  .msg-date-sep {
    text-align: center; font-size: 11px; font-weight: 700; color: #9B8F7A;
    padding: 12px 0 4px; letter-spacing: .06em;
  }
  .msg-bubble-row {
  ...
  .msg-bubble {
    padding: 10px 14px; border-radius: 16px; border: 2px solid #111110;
  ...
  .msg-bubble-row.mine .msg-bubble-meta { justify-content: flex-end; }
  ...
  .msg-templates-grid { display: flex; flex-wrap: wrap; gap: 6px; }
  .msg-template-btn {
    padding: 5px 12px; background: #F5F0E8; border: 1.5px solid #E8E0D0;
    border-radius: 99px; font-family: inherit; font-size: 12px; font-weight: 700;
  ...
  .msg-input-area {
    ...
    padding: 12px 16px; border-top: 2px solid #E8E0D0; background: #fff;
  }
  .msg-template-toggle {
  ...
  .msg-spinner { width: 24px; height: 24px; border: 3px solid #E8E0D0; border-top-color: #111110; border-radius: 50%; animation: spin .7s linear infinite; }
  .msg-spinner-sm { width: 16px; height: 16px; border: 2px solid rgba(0,0,0,.2); border-top-color: #111110; border-radius: 50%; animation: spin .7s linear infinite; }
