...
interface FiltersProps { currentStatus: string; currentSearch: string; currentView: string; counts: Record<string, number>; }
const tabs = [
  { value: "all", label: "All" },
...
export default function ArtworkFilters({ currentStatus, currentSearch, currentView, counts }: FiltersProps) {
  ...
  function update(key: string, value: string) {
    ...
    const srch = key === "search" ? value : currentSearch;
    const view = key === "view" ? value : currentView;
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div className="flex flex-wrap gap-1.5">
        {tabs.map((tab) => (
          <button key={tab.value} onClick={() => update("status", tab.value)}
      ...
      <div className="flex items-center gap-2">
        <div className="relative">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
            onKeyDown={(e) => { if (e.key === "Enter") update("search", search); }}
